function [ vep_data, digital_data, pre, post, decimate_rate ] = load_vep(filename, on_off)
...
if (decimate_rate ~= 1)
  cleanDigitalIn = cleanDigitalIn(1:decimate_rate:end);
end
...
preEventPlot_samples = floor(preEventPlot_sec * fs / decimate_rate);
% # of samples to extend plot following event time
postEventPlot_samples = floor(postEventPlot_sec * fs / decimate_rate) - 1;
...
for ii = 1:size(data, 1)
    ...
    if (decimate_rate ~= 1)
      chData = decimate(chData, decimate_rate);
    end
