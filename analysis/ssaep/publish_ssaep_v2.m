...
%fgh = figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1], 'Visible', 'off');
...
for ii=1:length(channelToPlot)
    ...
    f_plot{ii} = f_plot{ii}(1:end-1);
    temp_plot{ii} = temp_plot{ii}(1:end-1);
...
%for sp = 1:3
    ...
    if ~isempty(strfind(tmpCmt{1}(9:25), '86'))
        for h=1:floor(f(end)/86)
            harmonics = [harmonics (h*86)];
            color = baseColor;
            if any(abs(mod((h*86), 60) + [0 -60]) < threshold)
                color = mergedColor;
            end
            plot([(h*86) (h*86)], ylim,'Color',color,'linewidth',1)
        end
    end
...
%set(gca, 'XTick', roundn([0 harmonics([1 2:2:end])], -1));
set(gca, 'XTick', roundn([harmonics([1:7:end])], -1));


%subplot(3,1,1);
%tmp = title(['SSAEP: '  allData(i)]);
%set(tmp,'interpreter','none');
%set(gca, 'TickDir', 'out')
...
%tmp = title(sprintf('SSAEP: %s, window: %f secs, overlap: %f, %s, start: %f sec, end: %f sec\n%s', name, windlengthSeconds, noverlapPercent, rabbit_ID, t2 / fs, t3 / fs, allData{i}), 'Interpreter', 'None');
tmp = title(sprintf('SSAEP: %.1f to %.f seconds', cutLengthB / fs, cutLengthE / fs), 'Interpreter', 'None');
%tmp = title(sprintf('SSAEP: window: %0.1f secs, overlap: %0.2f, %s, start: %0.1f sec, end: %0.1f sec\n%s', windlengthSeconds, noverlapPercent, rabbit_ID, t2 / fs, t3 / fs, allData{i}), 'Interpreter', 'None');
