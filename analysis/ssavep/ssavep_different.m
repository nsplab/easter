function [] = ssavep_different(rabbit_ID, ssavep1, index1, ssavep2, index2)
...
pathname1 = ['../../../../data/easter/' rabbit_ID '/neuro/binary_data/' ssavep1 '/'];%path for the data in easter binary format
pathname2 = ['../../../../data/easter/' rabbit_ID '/neuro/binary_data/' ssavep2 '/'];%path for the data in easter binary format
...
pathname_matdata = ['../../../../data/easter/' rabbit_ID '/neuro/matlab_data/' ssavep1 '/'];
pathname_matdata = ['../../../../data/easter/' rabbit_ID '/neuro/matlab_data/' ssavep2 '/'];

[ channelNames, gtechGND, earth ] = rabbit_information(rabbit_ID);

[ S1, allData1 ] = get_information(pathname1, pathname_comments, upper(ssavep1));
[ S2, allData2 ] = get_information(pathname2, pathname_comments, upper(ssavep2));
...
fprintf('%s: %s\n', S1{index1}, allData1{index1});
fprintf('%s: %s\n', S2{index2}, allData2{index2});

[ data1, cleanDigitalIn1 ] = load_data([pathname1 S1{index1}], maxNumberOfChannels, digitalInCh, channelNames);
[ data2, cleanDigitalIn2 ] = load_data([pathname2 S2{index2}], maxNumberOfChannels, digitalInCh, channelNames);
...
comment1 = allData1(index1);
comment2 = allData2(index2);

[ nominal_frequency, frequency ] = get_frequency(comment1, S1{index1});


[ nominal_frequency1, frequency1 ] = get_frequency(comment1, S1{index1});
[ nominal_frequency2, frequency2 ] = get_frequency(comment2, S2{index2});
...
for ii=1:numel(channelToPlot)
    ...
    [lpxx, f, lpxxc] = pwelch(chData1, blackmanharris(windlength), noverlap, windlength, fs, 'ConfidenceLevel', 0.95);
    ...
    %[lpxx, f, lpxxc] = pwelch(chData2(1:round(3.5*9600)), blackmanharris(windlength), noverlap, windlength, fs, 'ConfidenceLevel', 0.95);
    [lpxx, f, lpxxc] = pwelch(chData2(cutLengthE2:end), blackmanharris(windlength), noverlap, windlength, fs, 'ConfidenceLevel', 0.95);
...
savename = sprintf('%s_%s_%s_%d_%d', rabbit_ID, ssavep1, ssavep2, index1, index2);
%savename = sprintf('%s_%s_%s_%d_%d_not_subtracted', rabbit_ID, ssavep1, ssavep2, index1, index2);
