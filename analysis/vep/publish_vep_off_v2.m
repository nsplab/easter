...
event_index_LED_ON = find(diff_cleanDigitalIn==LED_OFF_edge);               %find the edges that correspond to LED turning on
...
for ii=1:length(channelToPlot)                                                      %for each channel
    ...
    if publication_quality == 1 || publication_quality == 3
        confMean = bootci(100, @mean, chData_all_single_trial_collection_ON);
        %size(chData_all_single_trial_collection_ON)
